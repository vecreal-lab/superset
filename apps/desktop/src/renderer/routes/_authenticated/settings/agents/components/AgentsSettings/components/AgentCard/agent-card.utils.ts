...
export function getPreviewNoPromptCommand(preset: ResolvedAgentConfig): string {
	if (preset.kind !== "terminal") {
		return "Factory Chat opens a chat pane without a shell command.";
	}
...
export function getPreviewTaskCommand(preset: ResolvedAgentConfig): string {
	if (preset.kind !== "terminal") {
		return preset.model
			? `Factory Chat opens with model ${preset.model}.`
			: "Factory Chat opens with the rendered task prompt.";
	}
